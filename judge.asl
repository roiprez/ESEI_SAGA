...
//Devuelve el entero que corresponde al dueño de la jugada
plNumb(A,PlNumb):-
	(A == player1 & PlNumb = 1) | (A == player2 & PlNumb = 2).

//Devuelve las coordenadas NX,NY del siguiente movimiento a realizar en función 
//de las coordenadas de la ficha a mover y la dirección
...
+!startGame : actual(Player) & contador(N) & size(Size) & N>=0 <-
	//Creamos el tablero cuando se inicia la partida
	if(N == 100){
		for ( .range(I,0,(Size-1)) ) {
			for ( .range(J,0,(Size-1)) ) {
				.random(Color,10); 
				+tablero(ficha(math.round(5*Color),in),celda(I,J,0));
			};
		 };
		 -+contador(N-1);
		 !startGame;
	 /* Si el tablero ya está creado buscamos todas las creencias de tipo tablero
	 * y las vamos introduciendo en una lista que luego recorremos para enviar al jugador,
	 * el jugador entonces recibe la orden de mover, y una vez lo hace se le retiran las creencias 
	 * que tenía del tablero */
	} else {
		-+contador(N-1);
		.print("Turnos restantes: ", N);
...
+moverDesdeEnDireccion(pos(P1,P2),Dir)[source(A)] : actual(A) & nextMove(P1,P2,NX,NY,Dir) & plNumb(A,PlNumb) & valido(P1,P2,Dir) & tablero(X,celda(J,K,Own))  <- 
	...
	.print(tablero(X1,celda(P1,P2,Own1)));
	.print(tablero(X2,celda(NX,NY,Own2)));	
	-tablero(X1,celda(P1,P2,Own1));                         
	-tablero(X2,celda(NX,NY,Own2));
	+tablero(X1,celda(NX,NY,PlNumb));		
	+tablero(X2,celda(P1,P2,PlNumb));
